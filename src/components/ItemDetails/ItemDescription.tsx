...
export const ItemDescription: React.FC<ItemDescriptionProps> = ({
	...
	useEffect(() => {
		const fetchShippingAddress = async () => {
			try {
				const response = await httpRequestService.fetchApi<TShippingAddress[]>(
					"/shipping-address/me"
				);
		...
		await createOrder(
			{
				...
				shippingDuration: params.shippingDuration,
				shippingPrice: params.shippingCharges,
			},
			params.price,
			params.currencyType
		);
			...
			<div className="flex flex-col md:flex-row md:gap-6 md:items-center">
				...
				<div className="flex gap-2 items-center">
					<p className="text-sm font-[400] text-[#6B6F93]">
						{params.shippingType === "LOCAL"
							? `Local Shipping (${params.address})`
							: "Global Shipping"}
					</p>
				</div>
			</div>
