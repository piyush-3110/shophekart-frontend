...
export default function useCreateOrder(walletAddress: `0x${string}`) {
    ...
    async function createOrder({ ...data }: TCreateOrderParams, price: number, currencyType: TCurrencyType) {
        setIsLoading(true)
        setIsSuccess(false)
        try {
            ...
            setOrder(order)
            try {
                await createOrderOnChain({ currencyType, price, productIdOnChain: data.productIdOnChain, shippingPrice: data.shippingPrice })

                setIsSuccess(true)
            } catch {
                toast({
                    title: "There was an error plaicing order",
                    variant:"destructive"
                })
                setIsSuccess(false)
                setIsLoading(false)
    ...
    useEffect(() => {
        ...
        if (isSuccess && nftIds?.length && nftIds.length > 0 && order ) {
            (async () => {
                ...
                try {
                    await OrderService.udpateOrderNftId({ orderId: order._id, nftId: Number(stringNftId) })
                    toast({
